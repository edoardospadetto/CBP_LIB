...
do kk = 1,100
    ...
    call cbp_Bidiag(localA, descriptorA, localR, descriptorR, localL, & 
  	...
  	if(rank .eq. 0) middle = MPI_Wtime();

     call cbp_SVD_GulobReinsch( localA, descriptorA, dimA, singular_values, & 
    							descriptorSV, rowproc, colproc,rank, nprocs)  
    ...
    deallocate (localL)
 	...
 	if(rank .eq. 0) print*, dimA(1) , finish -start, middle-start
