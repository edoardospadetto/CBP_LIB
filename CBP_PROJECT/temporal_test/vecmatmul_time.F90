...
program passm
    ...
    integer , dimension(2):: descriptorv, descriptorres
    ...
    CTYPE , allocatable, dimension(:) :: localv, localres , res1 , res2, v
...
do kk = 1,20,1
  ...
  call cbp_init_1D_descriptor(descriptorv, dimA(2), rank, nprocs)
  call cbp_init_1D_descriptor(descriptorres, dimA(1), rank, nprocs)


!  v(:descriptorv(1)) = localv(:descriptorv(1))


!  call cbp_redispatch_1d(v,descriptorv,[dimA(2),1], rank, nprocs)



!  call GetBackMatrix(localA, descriptorA, A, rowproc, colproc, 	idxrow, idxcol,rank)
!  call cbp_redispatch_1d(res2,descriptorres,[dimA(1),1],rank,nprocs)
    ...
    if (rank .eq. 0)  t1 = MPI_Wtime()
       ...
       call cbp_vecmatmul(localA, descriptorA, localv, descriptorv, localres, descriptorres ,&
                          rank, rowproc,colproc, context, .false.)
