...
! Send a row of a distributed matrix.
! if slice_idx = 0 send the descriptor
...
!Tag -> A number from 0 to 10 Don't worry its made unique for this step. 
    ...
    subroutine MPI_comm_wrap_row(LocalMatrix, temp_buffer, MatrixDescriptor, temp_desc,& 
                      merow,mecol,otherrow,othercol,rowproc,colproc,& 
                      mods,tag, slice_idx, requestM, requestD)
        
        implicit none
        CTYPE, dimension(:,:) :: LocalMatrix
        CTYPE, dimension(:) :: temp_buffer 
        integer, dimension(4) :: MatrixDescriptor, temp_desc
        integer :: slice_idx 
        integer :: dims(2) 
        ...
        if(dims(2) .ne. size(temp_buffer) ) then 
                print*, "Invalid Size of Temporary buffer"
        end if         
        if (mods .eq. 's') then
            if (slice_idx .gt. 0) then
            call MPI_ISEND( LocalMatrix(slice_idx, : ), dims(2), MPI_CTYPE,&
                     grid2rank(otherrow,othercol,rowproc, colproc) , & 
                     100*grid2rank(otherrow,othercol,rowproc,colproc)+50*tag+mod(slice_idx,30) ,&  !tag is the  dest
                     MPI_COMM_WORLD, requestM, ierr)
                     print*, "sending",'||', LocalMatrix(slice_idx, 1:3),'||', otherrow,othercol, slice_idx, dims(2)
            else if (slice_idx .eq. 0) then     
            call MPI_ISEND( MatrixDescriptor, 4, MPI_INTEGER,&
                     grid2rank(otherrow,othercol,rowproc, colproc) , & 
                     100*grid2rank(otherrow,othercol,rowproc,colproc)+50*tag+31 ,&  !tag isthedest
                     MPI_COMM_WORLD, requestD, ierr)
            end if
            call mpic_checkerror(ierr)
        else if (mods .eq. 'r') then
            if (slice_idx .gt. 0) then
            call MPI_IRECV( temp_buffer, dims(2), MPI_CTYPE,&
                     grid2rank(otherrow,othercol,rowproc,colproc) , & 
                     100*grid2rank(merow,mecol,rowproc,colproc)+50*tag+mod(slice_idx,30) ,& !tagisthe receiver, so me
                     MPI_COMM_WORLD, requestM, ierr)
            else if(slice_idx .eq. 0) then         
            call MPI_IRECV( temp_desc, 4, MPI_INTEGER,&
                     grid2rank(otherrow,othercol,rowproc,colproc) , & 
                     100*grid2rank(merow,mecol,rowproc,colproc)+50*tag+31 ,& !tagishereceiver, so me
                     MPI_COMM_WORLD, requestD, ierr)
            end if
            call mpic_checkerror(ierr)
        else 
...
subroutine ComputeMatmul(localA, localB, localC, descriptorA, descriptorB , descriptorC, rowproc, colproc, rank, process_grid,& 
    ...
    CTYPE, dimension(:) , allocatable :: temp_bufferA, temp_bufferB
    integer, dimension(4) :: temp_descriptorA, temp_descriptorB
    ...
    integer :: idxrow, idxcol
    integer :: withwhorowra, withwhocolra, withwhorowrb, withwhocolrb
    integer :: withwhorowsb, withwhocolsb, withwhorowsa, withwhocolsa
    ...
    allocate(temp_bufferA(size(locala, dim=2)) ) 
    allocate(temp_bufferB(size(localb, dim=2)) )  
    ...
    do ii = 2,2
        ...
        !############################################################################################################!
        !send B -------------------------------------------------------------------------------------------------------
        call find_coop(old_process_grid, process_grid, idxrow, idxcol, withwhorowsb, withwhocolsb, rowproc, colproc, "sb")
        !send A--------------------------------------------------------------------------------------------------------
        call find_coop(old_process_grid, process_grid, idxrow, idxcol, withwhorowsa, withwhocolsa, rowproc, colproc, "sa")
        !recv B--------------------------------------------------------------------------------------------------------
        call find_coop(old_process_grid, process_grid, idxrow, idxcol, withwhorowrb, withwhocolrb, rowproc, colproc, "rb")
        !recv A--------------------------------------------------------------------------------------------------------
        call find_coop(old_process_grid, process_grid, idxrow, idxcol, withwhorowra, withwhocolra, rowproc, colproc, "ra")
        ...
        !Iterate through rows, send it, receive in temp buffer, overwrite, and go on.. 
        do slice = 0, 1!max(size(localA, dim = 1) , size(localB, dim= 1 )) 
        

       

        if (slice .le. size(localb, dim = 1)) then 
        if ((withwhorowsb .gt. 0 ).and. (withwhocolsb .gt. 0 )) then
            if((withwhorowsb .eq. idxrow) .and. (withwhocolsb .eq. idxcol)) then 
                !DO NOTHING
               
            else 
            sb = .true.
            call  MPI_comm_wrap_row(LocalB ,temp_bufferB,descriptorB,temp_descriptorB,idxrow,idxcol,& 
                withwhorowsb, withwhocolsb,rowproc,colproc,'s',1, slice,requestsendB, requestsendDB)
       ...
       if (slice .le. size(locala, dim = 1)) then 
       if ((withwhorowsa .gt. 0 ).and. (withwhocolsa .gt. 0 )) then
        
            if((withwhorowsa .eq. idxrow) .and. (withwhocolsa .eq. idxcol)) then 
                !DO NOTHING
            else 
            sa = .true.
            
            call  MPI_comm_wrap_row(LocalA ,temp_bufferA,descriptorA,temp_descriptorA,idxrow,idxcol,&
                withwhorowsa, withwhocolsa,rowproc,colproc,'s',0,slice,requestsendA, requestsendDA) 
            end if
        end if
        end if
        !############################################################################################################!
        ...
        !############################################################################################################!
        if (slice .le. size(localb, dim = 1)) then
        if ((withwhorowrb .gt. 0 ).and. (withwhocolrb .gt. 0 )) then
            if(.not. ( (withwhorowrb .eq. idxrow) .and. (withwhocolrb .eq. idxcol) ) ) then 
                rb = .true.
                
                call MPI_comm_wrap_row(LocalB ,temp_bufferB,descriptorB,temp_descriptorB, idxrow,idxcol,& 
                    withwhorowrb, withwhocolrb,rowproc,colproc,'r',1, slice, requestrecvB, requestrecvDB)
        ...
        !############################################################################################################!
        if (slice .le. size(locala, dim = 1)) then
        if ((withwhorowra .gt. 0 ).and. (withwhocolra .gt. 0 )) then
            
            if(.not. ( (withwhorowra .eq. idxrow) .and. (withwhocolra .eq. idxcol) ) ) then 
            ra = .true.
            
            call MPI_comm_wrap_row(LocalA ,temp_bufferA, descriptorA,temp_descriptorA, idxrow,idxcol,& 
                withwhorowra, withwhocolra,rowproc,colproc,'r',0,slice, requestrecvA, requestrecvDA)
        ...
        !if (rank .eq. 0 ) then
        !        print*, slice, size(locala, dim = 1 ) , size(localb, dim = 1)   
        !end if
        
        if(sb .and. slice .le. size(localB , dim = 1) ) then
            if(slice .eq. 0) then 
        ...
        if(sa .and. slice .le. size(localA , dim = 1)) then
            if(slice .eq. 0) then 
        ...
        if (ra .and. slice .le. size(localA , dim = 1))  then 
            if(slice .eq. 0) then
            call MPI_WAIT(requestrecvDA, MPI_STATUS_IGNORE, ierr )
            !print*, rank, descriptorA, "||", temp_descriptorA
            descriptorA  = temp_descriptorA
            else
            call MPI_WAIT(requestrecvA, MPI_STATUS_IGNORE, ierr )
            localA(slice,:) = temp_bufferA
            end if

        end if
        if (rb .and. slice .le. size(localB , dim = 1) ) then 
            if(slice .eq. 0) then
            call MPI_WAIT(requestrecvDB, MPI_STATUS_IGNORE, ierr )
            descriptorB = temp_descriptorB 
            else
            call MPI_WAIT(requestrecvB, MPI_STATUS_IGNORE, ierr )
            !print*, sum(temp_bufferB(:descriptorB(2))/descriptorB(2))
            print*,"received", '||', temp_bufferB(1:3),'||', idxrow, idxcol, slice
            localB(slice,:) = temp_bufferB
        ...
        call cbpfor_auto_gemm(localA(:descriptorA(1),:descriptorA(2)),& 
                                  localB(:descriptorB(1),:descriptorB(2)),& 
                                  localC(:descriptorC(1), :descriptorC(2)), 0, ngpu, cpuratio, handle) 
   ...
   ! end if 
    ...
    deallocate(temp_bufferA, temp_bufferB)  
