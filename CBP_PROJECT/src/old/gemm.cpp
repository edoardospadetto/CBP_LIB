...
template <typename T>
int cbp_auto_gemm(T* A, T*B , T*C, int* dimA, int* dimB, int* dimC, T alpha, T beta, float cpuratio, cublasXtHandle_t xt_ )
{
	  ...
	  //if(cublasXtCreate(&xt_) != CUBLAS_STATUS_SUCCESS) {printf("handle create fail\n"); return 1;}
  	  ...
  	  cbp_Gemm(A,B,C,dimA[0],dimB[1],dimA[1],xt_,alpha,beta, cpuratio);
